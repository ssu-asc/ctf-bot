...
test('copy migration edits tracked welcomes once, keeps message IDs and preserves manually edited topics', async () => {
  ...
  for (const field of f.config.fields) {
    const state = f.values.get(field.key); state.copyVersion = 2;
    ...
    f.channels.find(c => c.id === state.channelId).topic = field.key === 'web'
      ? `${field.emoji} ${field.name}에 관심 있는 멤버들이 주제를 제안하고 팀을 찾는 곳입니다. 팀은 2~3명 권장, 최대 4명이며 PM 1명을 정해 주시기 바랍니다.`
      : '운영진이 직접 정한 채널 설명';
  }
  ...
  assert.ok(f.channels[1].topic.endsWith('정해주세요.'));
